...
<form class="form-inline" id="search_form_${tableDefine.id}" onsubmit="return false;">
 	...
 	list.each{
		...
		<div class="form-group">
			<label for="${column.dataName}_search">${column.cnname}</label>
				...
				<input type="text"  name="${column.dataName}" id="${column.dataName}_search" class="form-control input-medium" placeholder="请输入${column.cnname}">
				<%
				}
				%>
		</div>&nbsp;&nbsp;
